...
+ 隔离型：几个能保证一致性和原子性的事务并发执行也可能会导致不一致的状态，如果一个事务在另一个并发运行的事务的中间不一致时刻也进行同一数据的更新操作，即使两个事务都完成后，数据库仍可能处于不一致状态。一种避免事务并发执行而产生问题的途径是串行地执行事务，然后事务并发执行能显著地改善性能。事务的隔离性确保事务并发执行后的系统状态与这些事务以某种次序一个接一个地执行后的状态是等价的。确保隔离性是数据库系统中称作并发控制系统（concurrency-control system）的部件的责任。

### （2）存储结构

+ 易失性存储器。
+ 非易失性存储器。
+ 稳定性存储器（stable storage）。稳定性存储器中的信息*永远*不会丢失。理论上不可能获得稳定性存储器，但可以通过技术近似使得数据丢失的可能性微乎其微。为了一个事务能够持久，他的修改应该应该写入稳定性存储器；为了一个事务是原子的，日志记录需要在对磁盘上的数据库做任何改变之前写入稳定性存储器。显然，一个系统保证的持久性和原子性的程度取决于稳定性存储器的实现到底有多稳定。
...
&emsp;&emsp;如何确定一个调度是可串行化的？当 I 与 J 是不同事务在相同的数据项上的操作，并且其中至少有一个是 write 指令时，我们说 I 与 J 是冲突的。设 I 与 J  是调度 S 的两条连续指令，若 I 与 J 属于不用事务的指令且不冲突，则可以交换 I 与 J 的顺序得到一个新的等价调度 S'。重复这一步骤得到一个与调度 S 等价的串行调度，该等价意味着不管初始系统状态如何，调度 S 将与某个串行调度产生相同的最终状态。如果调度 S 可以经过一系列非冲突指令交换转换成 S'，我们称 S 与 S' 是冲突等价（conflict equivalent）的。冲突可串行化：若一个调度 S 与一个串行调度冲突等价，则称调度 S 是冲突可串行化（conflict serializable）的。
