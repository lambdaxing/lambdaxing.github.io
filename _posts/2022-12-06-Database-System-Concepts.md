...
&emsp;&emsp;该章描述：1.物理存储介质 2.将由于故障而引起数据丢失的可能性最小化的机制 3.记录是如何映射到文件然后又如何映射到磁盘中的比特的。
...
&emsp;&emsp;许多查询只涉及了文件中的一小部分记录。索引是一种有助于无须检查所有记录而快速定位所需关系记录的结构。该章描述数据库系统使用的几种索引类型。
...
+ 顺序索引。基于值的顺序排序。
+ 散列索引。基于将值平均分布到若干散列桶中，通过散列函数（hash function）得到一个值所属的散列桶。
...
&emsp;&emsp;当文件中有记录插入或删除时，或者文件中记录的任何搜索码属性更新时，索引都需要更新。对记录的更新可以设计为对旧记录的删除和新记录的插入。单级索引的更新算法如下，多级索引的插入和删除算法可以采用同样的技术更新更高层的索引。
...
&emsp;&emsp;辅助索引（非聚集索引）必须是稠密索引，对每个搜索码值都有一个索引项，而且对文件中的每条记录都有一个指针。而聚集索引可以是稀疏索引，可以只存储部分搜索码值，通过顺序扫描文件总可以找到对应的记录。辅助索引的结构和聚集索引不同，特别是在搜索码不是一个候选码时，因为记录按聚集索引而不是辅助索引的搜索码顺序存放，辅助索引必须包含指向每一条记录的指针。我们可以用一个附加的间接指针层来实现非候选码的搜索码上的辅助索引，辅助索引中的指针不直接指向文件，而是指向一个包含文件指针的桶。

![11_6](/assets/img/2022-12-06-Database-System-Concepts/11_6.png)
...
&emsp;&emsp;一个搜索码可以有多个属性，一个包含多个属性的搜索码称为复合搜索码（composite search key）。除了搜索码是属性列表，按字典序排序外，其余的结构和其他搜索码一样。一个在复合码上的有序索引可以用来有效地查找某些类型的查询。

### （3）B+ 树索引文件

&emsp;&emsp;随着文件的增大、记录的删除和更新，顺序索引的查找性能和数据扫描性能都会下降，因此需要频繁地对文件进行重新组织。B+ 树（B+ Tree）索引结构是在数据**插入**和**删除**的情况下仍能保持其执行效率的几种使用最广泛的索引结构之一。B+树采用平衡树结构，其中根到叶结点的每条路径的长度相同。每个非叶结点有 ⌈n/2⌉ ~ n 个孩子结点，其中 n 对特定的树是固定的。

&emsp;&emsp;B+树结构会增加文件插入和删除的性能开销和空间开销，但能减小文件重组的代价，整体带来的性能提高使得更多的空间开销也是可以接受的。

&emsp;&emsp;典型的B+树结点结构最多包含 n-1 个搜索码值 K1，K2，...，Kn-1，以及 n 个指针 P1，P2，...，Pn。每个结点中的搜索码值按值排序存放，即 i < j，则 Ki < Kj（假设没有重复的码值）。

![11_7](/assets/img/2022-12-06-Database-System-Concepts/11_7.png)

&emsp;&emsp;叶结点（leaf node）的结构中，对 i = 1，2，...，n-1，指针 Pi 指向具有搜索码值 Ki 的一条文件记录。每个叶结点最多可有 n-1 个值，允许叶结点包含的值的个数最少为 ⌈(n-1)/2⌉。因为各叶结点之间按照所含搜索码值大小有一个线性的顺序，我们可以用指针 Pn 将叶结点按搜索码顺序串在一起，以便对文件进行高效的顺序处理。

![11_8](/assets/img/2022-12-06-Database-System-Concepts/11_8.png)

&emsp;&emsp;B+树的非叶结点（nonleaf node）形成叶结点上的一个多级（稀疏）索引。非叶结点的结构和叶结点相同，只不过非叶结点中所有的指针都是指向树中结点的指针。一个非叶结点可以容纳 ⌈n/2⌉ ~ n 个指针。结点的指针数称为该结点的扇出。非叶结点也称为内部结点（internal node）。

&emsp;&emsp;注意到，B+树索引不仅采用平衡树结构，同时也采用了搜索树的结构，即内部结点中的指针 Pi 指向一棵所含搜索码值大于等于 Ki 且小于 Ki+1 的子树。 

![11_9](/assets/img/2022-12-06-Database-System-Concepts/11_9.png)

&emsp;&emsp;B+树从根到叶结点的每条路径长度都相同，即都是平衡（Balanced）的，正是这一性质保证了B+树索引有良好的查找、插入和修改性能。

&emsp;&emsp;B+树的查询。

![11_3_2](/assets/img/2022-12-06-Database-System-Concepts/11_3_2.png)

&emsp;&emsp;B+树的更新。
+ 插入。一个索引项要插入到 B+ 树中，首先找到该索引项需插入的叶结点。如果叶结点未满则直接插入，如果叶结点已满则需要拆分该叶结点，并将拆分后用于区分旧结点和新结点的项（内部结点的项）插入到父结点（内部结点）中，如果父结点满了则需要拆分父结点，重复这一过程直到插入成功，如果一直到达根结点且根结点也满了，则根结点分裂成内部结点，一个新的根结点被创建并包含指向原根结点分裂后的内部结点的指针的项。当叶结点满时，索引项的插入可以在拆分前也可以在拆分后，取决于具体实现。
+ 删除。删除一个索引项也需要首先找到该索引项所在的叶结点，然后删除该叶结点中对应的项。如果删除后叶结点的项数低于B+树中叶结点要求的最小项数，则该叶结点需要和兄弟结点合并或重新分配（除非该叶结点是一个根结点）。当两个结点的项能放到一个结点中时才能合并，合并后有一个结点将被删除，而其父结点中指向该结点的对应项也需要删除，如果父结点删除该项后不满足B+树内部结点要求的最小项数，则该父结点也需要合并或重新分配，重复这一过程直到删除成功。当到达根结点时，根结点只要满足拥有至少2个项，一个项的根结点将被它的唯一项的结点取代并删除。当两个结点的项无法放到一个结点中时要执行重新分配，重新分配可以实现为从兄弟结点中借一个项，使得被删除了一个项的结点满足最小项数要求，这同时需要更新父结点中的某个项，该项用于区分那两个重新分配的儿子结点。

&emsp;&emsp;一个关系拥有多个包含同一搜索码值的记录，即两条或多条记录在索引属性上拥有相同的值，该搜索码称为不唯一搜索码（nonunique search key）。不唯一搜索码在记录删除方面存在效率问题，需要比对每一个相同搜索码的索引项，可以通过创建包含原始搜索码和其他属性的复合搜索码来确保搜索码唯一，对于所有记录，该复合搜索码是唯一的，这也通常被大多数数据库系统使用。这个额外属性称为唯一化（uniquifier）属性。另一种方法是为不唯一搜索码维护一个记录指针的桶（或者列表）。这种方法在空间上更有效，但是带来较多的复杂性。

&emsp;&emsp;B+ 树的插入和删除操作复杂，但需要的 I/O 操作较少。在最坏情况下，一次插入/删除的 I/O 操作数可能正比于以 n/2 为底 N 的对数，其中 n 是结点中的指针的最大值，N 是索引文件中的记录条数。换句话说，以 I/O 操作来衡量的插入和删除操作的代价是和 B+ 树的高度成正比的，因此是比较低的，这也使得 B+ 树在数据库实现中成为频繁使用的索引结构。大部分非叶结点存在于数据库缓冲区中，这是因为内部结点的访问比叶结点多很多倍，且内部结点的数量比叶子结点少很多倍。因此，一次查询通常只需要一两次 I/O 操作，在插入时叶结点分裂的可能性也比较小。

&emsp;&emsp;随着在树中不断进行插入和删除，B+ 树结构中初始建立在一组记录上基本连续的叶结点块也会逐渐丢失连续性，使得相邻的叶结点在磁盘上实际并不相邻，对叶结点的顺序访问也需要等待磁盘寻道时间。为了恢复顺序性也需要对索引进行重建。

### （4）B+ 树扩展

+ B+ 树文件组织。B+ 树除了作为索引能解决索引查找性能下降的问题，还可以作为一个文件中的记录的组织者，用 B+ 树的叶级结点组织存放实际记录的磁盘块。在 B+ 树文件组织（B+-tree file organization）中，树的叶结点存储的是记录而不是指向记录的指针。用 B+ 树作为文件组织方式，空间的利用尤为重要，因为记录所占的空间很可能比码的指针所占用的空间大得多。通过在分裂和合并结点时在重新分布中涉及更多的兄弟结点，可以改善 B+ 树的空间利用率。具体来说就是在插入已满结点时先和兄弟结点进行分配再插入，若兄弟结点也满了才进行分裂，并在待插入结点、兄弟结点和分裂结点这三个结点中均匀分配两个满结点的所有记录，因此每个结点大约 2/3 满。在删除记录时，结点项数少于 2n/3 则从兄弟结点借一项，若两兄弟也只有 2n/3 项，则删掉一个结点在剩下的两个结点中均匀分布少于 2n 的项。参与重新分布的兄弟结点越多，空间利用率越高，更新的代价也越高。B+ 树文件组织可以用于存储大型数据对象，大数据对象被拆分成稍小的记录序列并组织成 B+ 树文件组织进行存储。拆分的记录可以按序编号，或者根据记录在大对象中的字节偏移量进行编号，记录的编号便可用作搜索码。

    ![11_20](/assets/img/2022-12-06-Database-System-Concepts/11_20.png)

+ 辅助索引和记录重定位。尽管记录没有更新，B+ 树文件组织也可能会改变记录的位置（比如分裂、合并）。存储了指向重定位过的记录的指针的辅助索引因此必须更新，这个操作的代价及其高昂。因而，在辅助索引中一般存储主索引搜索码属性的值。这使得用辅助索引定位记录需要先用辅助索引找到主索引搜索码的值，然后用主索引来找到对应的记录。
+ 字符串上的索引。字符串是变长的且可能会很长，这将导致 B+ 树索引的分裂合并更加复杂、结点扇出降低以及相应地增加树的高度。使用前缀压缩技术，不用在非叶结点存储整个搜索码值，只需存储每个搜索码值的一个前缀，这个前缀足以将两个子树中的码值区分开。
+ B+ 树索引的批量加载。将大量项一次插入到索引中称为索引的批量加载（bulk loading），这不同于从磁盘中不同位置一次次取出再一次一次插入索引这种耗时的方法。先构建一个含有关系索引项的临时文件，然后根据索引搜索码来排序文件，然后扫描排序好的文件并将项插入到索引中，排序算法所需的时间和 I/O 代价与读几次文件相当。自底向上 B+ 树构建通过对项进行排序后，将项分解到块中产生 B+ 树的叶级，每个块中的最小值以及指向块的指针用来构建下一级的 B+ 树项，如此反复直到创建根结点。在一个关系上创建索引时，大多数数据库系统实现了基于项排序和自底向上构建的有效技术。如果一次插入非常多的元组到已存在的关系中，一些数据库系统会先删除在该关系上的索引（除主码上的索引），在插入后使用批量技术重新构建。
+ B 树索引文件。B 树去除了搜索码值存储中的冗余，即只允许搜索码值出现一次，要么在内部结点，要么在叶子结点上。

    ![11_21](/assets/img/2022-12-06-Database-System-Concepts/11_21.png)

+ 闪存。闪存存储通过块来组织的，B+ 树索引结构可以在闪存存储中使用。这加快了查询的访问速度，只是闪存中的 B+ 树结点大小通常比磁盘小得多。

### （5）多码访问

&emsp;&emsp;索引可以加速查询，一个查询如果存在多个索引则使用多个索引，或者使用建立在多属性搜索码上的索引。

### （6）静态散列

&emsp;&emsp;顺序文件组织必须使用二分搜索或访问索引结构来定位数据，这将导致过多的 I/O 操作。基于散列（hashing）技术的文件组织能够避免访问索引结构，散列也提供了一种构造索引的方法。

&emsp;&emsp;术语桶（bucket）表示能存储一条或多条记录的一个存储单位，通常一个桶就是一个磁盘块。令 K 表示所有的搜索码值的集合，令 B 表示所有桶地址的集合，散列函数（hash function）h 是一个从 K 到 B 的函数。在散列文件组织（hash file organization）中，通过计算所需记录搜索码值上的一个函数直接获得包含该记录的磁盘块地址。在散列索引组织（hash index organization）中，把搜索码以及与它们相关联的指针组织成一个散列文件结构。

&emsp;&emsp;理想的散列函数具有均匀、随机的分布特性。均匀是指为每个桶分配同样数量的搜索码值，随机是指即使搜索码有一些相关性，分布也将使得所有桶大致拥有相等的记录数。因此，散列函数不能和搜索码值扯上现实关系。通常散列函数在搜索码中字符的内部二进制表示上执行计算，一个简单函数是先计算码中字符的二进制表示的总和，然后返回该总和取桶数目的模。一个设计良好的散列函数一般情况下查找所花费的时间是一个较小的常数，而与文件中搜索码的个数无关。

![11_23](/assets/img/2022-12-06-Database-System-Concepts/11_23.png)

&emsp;&emsp;桶溢出（bucket overflow）发生原因：

![11_24](/assets/img/2022-12-06-Database-System-Concepts/11_24.png)

&emsp;&emsp;桶溢出的解决方案：
+ 闭地址（close addressing）散列结构的溢出桶/溢出链（overflow chaining）。
+ 开地址（opend addressing）散列结构的线性探查法（linear probing），即使用下一个（按轮转顺序）有空间的桶。开地址曾用于构造编译器和汇编器的符号表，而闭地址更适合用于数据库系统，原因在于开地址中的删除操作十分麻烦。编译器和汇编器只是在符号表上做查找和插入操作，而在数据库中，处理删除的能力和处理插入一样重要。

&emsp;&emsp;静态散列必须在实现系统时选择确定的散列函数，此后若被索引的文件变大或缩小，再想改变散列函数就不容易了。散列函数将搜索码值映射到桶地址的固定集合 B 上，而过大的 B 会浪费空间，过小的 B 会发生桶溢出。

&emsp;&emsp;散列索引（hash index）将搜索码及其相应的指针组织成散列文件结构。散列函数作用于搜索码以确定对应的桶，搜索码和相应的指针被存入此桶中。

![11_25](/assets/img/2022-12-06-Database-System-Concepts/11_25.png)

&emsp;&emsp;术语散列索引表示散列文件结构，同时也表示辅助散列索引。散列索引只是一种辅助索引结构，因为如果一个文件自身是按散列组织的，就不必在其上另外建立一个独立的索引结构（聚集索引），因为散列文件组织能像索引那样提供对记录的直接访问，也可以认为以散列形式组织的文件上已经有了一个聚集散列索引。
...
&emsp;&emsp;构成单一逻辑工作单元的操作集合称作事务（transaction）。事务是访问并可能更新各种数据项的一个程序执行单元，用形如 begin transaction 和 end transaction 语句（或函数调用）来界定。事务是与数据库交互的一种理想方式，数据库系统需维护事务的以下 ACID 特性（ACID property）：
+ 原子性：事务的所有操作在数据库中要么全部正确地反应出来，要么完全不反应。
+ 一致性：隔离执行事务时（换言之，在没有其他事务并发执行的情况下）保持数据库的一致性。这里的一致性包括太过复杂而不能用 SQL 构建数据完整性且依赖于程序的一致性约束。这是编写事务的程序员的职责。
+ 隔离型：事务的正常执行不会被来自并发执行的其他数据库语句干扰，对多个事务的可能并发执行，系统保证在一个事务完成后另一个事务开始执行，或在一个事务开始前另一个事务已经完成执行。每个事务都感觉不到系统中有其他的事务在并发执行。
+ 持久性：一个事务成功完成后，它对数据库的改变必须是永久的，即使出现系统故障。

&emsp;&emsp;ACID 特性（ACID property）的再一次阐述：
+ 一致性：确保单个事务的一致性是编写该事务的应用程序员的责任，完整性约束的自动检查给这项工作带来了便利。
+ 原子性：由于故障，系统的状态不再反映数据库本应描述的现实世界的真实状态，则这种状态称为不一致状态（inconsistent state）。系统必然在事务执行中的某一时刻处于不一致状态，这种不一致状态除了在事务执行当中以外，在其他时刻是看不见的。这就是需要原子性的原因：如果具有原子性，某个事物的所有动作要么在数据库中全部反映出来，要么全部不反映。
    - 保证原子性的基本思路：对于事务要执行写操作的数据项，数据库系统在磁盘上的一个称为日志的文件中记录其旧值，如果事务没能完成，数据库系统从日志中恢复旧值，使得看上去事务从未执行过。保证原子性由数据库系统中称作恢复系统（recovery system）的组件处理。
+ 持久性：数据库恢复系统负责保证原子性和持久性。通过确保以下两条中的任何一条来达到持久性：
    - 事务做的更新在事务结束前已经写入磁盘。
    - 有关事务已执行的更新信息已写到磁盘上，并且此类信息足够充分，能让数据库在系统故障后重新启动时重新构造数据。
+ 隔离性：几个能保证一致性和原子性的事务并发执行也可能会导致不一致的状态，如果一个事务在另一个并发运行的事务的中间不一致时刻也进行同一数据的更新操作，即使两个事务都完成后，数据库仍可能处于不一致状态。一种避免事务并发执行而产生问题的途径是串行地执行事务，然而事务并发执行能显著地改善性能。事务的隔离性确保事务并发执行后的系统状态与这些事务以某种次序一个接一个地执行后的状态是等价的。确保隔离性是数据库系统中称作并发控制系统（concurrency-control system）的部件的责任。

### （2）存储结构

+ 易失性存储器。
+ 非易失性存储器。
+ 稳定性存储器（stable storage）。稳定性存储器中的信息*永远*不会丢失。理论上不可能获得稳定性存储器，但可以通过技术近似使得数据丢失的可能性微乎其微。为了一个事务能够持久，它的修改应该写入稳定性存储器；为了一个事务是原子的，日志记录需要在对磁盘上的数据库做任何改变之前写入稳定性存储器。显然，一个系统保证的持久性和原子性的程度取决于稳定性存储器的实现到底有多稳定。

### （3）事务原子性和持久性

&emsp;&emsp;恢复系统管理事务的典型方法是维护一个日志（log）。每个事务对数据库的修改都首先会记录到日志中，记录执行修改的事务标识符、修改的数据项标识符以及数据项的旧值和新值。然后数据库才会修改。维护日志提供了重做修改以保证原子性和持久性的可能，以及撤销修改以保证在事务执行发生故障时的原子性的可能。

&emsp;&emsp;事务未能成功执行完成称为中止（aborted）了，中止事物造成的变更被撤销称为事务已回滚（rolled back）。成功完成执行的事务称为已提交（committed）。撤销已提交事务所造成影响的唯一方法是执行一个补偿事务（compensating transaction），补偿事务的书写和执行是用户的的责任。

![14_1](/assets/img/2022-12-06-Database-System-Concepts/14_1.png)

### （4）事务隔离性和可串行化

&emsp;&emsp;允许并发执行事务的理由：
+ 提高吞吐量和资源利用率。利用 CPU 和 I/O 系统的并行性，多个事务并行执行，这增加了系统的吞吐量，即给定时间内执行的事务数增加。
+ 减少等待时间。并发执行共享CPU周期和磁盘存取，可以减少执行事务时不可预测的延迟，从而减少平均响应时间，即一个事务从提交到完成所需的平均时间。 

&emsp;&emsp;在数据库中使用并发执行的动机在本质上与操作系统中使用多道程序的动机是一样的。数据库系统有两种方法利用多处理器和多核的优势，一种是发现单个事务或查询内的并行性，另一种是支持大量并发的事务。数据库系统必须控制事务之间的交互，以防止它们破坏数据库的一致性，这是通过并发控制机制来保证的。

&emsp;&emsp;调度表示指令在系统中执行的时间顺序。一组事务的一个调度必须包含这一组事务的全部指令，并且必须保持指令在各个事务中出现的顺序。并发执行的控制完全由操作系统负责的话，会产生许多可能的调度，包括那些使数据库处于不一致状态的调度。保证所执行的任何调度都能使数据库处于一致状态，这是数据库系统中的并发控制部件的职责。在并发执行中，通过保证所执行的任何调度的效果都与没有并发执行的调度（串行调度）效果一样，则可以保证数据库的一致性。即，调度在某种意义上等价于一个串行调度，这种调度称为可串行化（serializable）调度。

&emsp;&emsp;如何确定一个调度是可串行化的？当 I 与 J 是不同事务在相同的数据项上的操作，并且其中至少有一个是 write 指令时，我们说 I 与 J 是冲突的。设 I 与 J  是调度 S 的两条连续指令，若 I 与 J 属于不同事务的指令且不冲突，则可以交换 I 与 J 的顺序得到一个新的等价调度 S'。重复这一步骤得到一个与调度 S 等价的串行调度，该等价意味着不管初始系统状态如何，调度 S 将与某个串行调度产生相同的最终状态。如果调度 S 可以经过一系列非冲突指令交换转换成 S'，我们称 S 与 S' 是冲突等价（conflict equivalent）的。冲突可串行化：若一个调度 S 与一个串行调度冲突等价，则称调度 S 是冲突可串行化（conflict serializable）的。


![14_10](/assets/img/2022-12-06-Database-System-Concepts/14_10.png)

![14_11](/assets/img/2022-12-06-Database-System-Concepts/14_11.png)

![14_12](/assets/img/2022-12-06-Database-System-Concepts/14_12.png)

### （5）事务隔离性和原子性

&emsp;&emsp;并发执行过程中事务故障也会影响到调度。如果事务失败了，必须撤销该事务的影响以确保其原子性，原子性要求依赖于该事务的任何事务（读取了前一个事务写的数据）也中止。

![14_14](/assets/img/2022-12-06-Database-System-Concepts/14_14.png)

### （6）事务隔离级别

![14_8_1](/assets/img/2022-12-06-Database-System-Concepts/14_8_1.png)

### （7）隔离性级别的实现

&emsp;&emsp;可以使用多种并发控制机制（concurrency- control schemer）保证，即使有多个事务并发执行时，不管操作系统在事务之间如何分时共享资源，都只产生可接受的调度。并发控制机制的目的是获得高度的并发行，同时保证所产生的调度是冲突可串行化或视图可串行化的、可恢复的、并且是无级联的。一些重要的并发控制机制的工作原理：
+ 锁。一个事务可封锁其访问的数据项，而不用封锁整个数据库。一个简单并且广泛用来确保可串行化的协议是两阶段封锁协议。它要求一个事务有两个阶段，一个阶段只获得锁但不释放锁，第二个阶段只释放锁但是不获得锁。两种锁模式：共享锁用于事务读的数据项，排他锁用于事务写的数据项。这两种锁模式以及两阶段封锁协议在保证可串行化的前提下允许数据的并发读。
+ 时间戳。为每个事务分配一个时间戳，对于每个数据项，系统维护两个时间戳，读时间戳记录读数据项的事务的最近时间戳，写时间戳记录写入该数据项当前值的事务的时间戳。时间戳用来确保在访问冲突的情况下，事务按照事务时间戳的顺序来访问数据项。
+ 多版本和快照隔离。

### （8）事务中的 SQL 语句表示

&emsp;&emsp;我们需要在用一系列 SQL 语句表示事务时保证 ACID 特性，而不像简单的读和写的模型。在两个事务的并发执行中，有些冲突在简单模型中无法发现，比如在查询和插入的并发执行中，查询结果取决于该插入是先于还是后于执行，这种情况称为幻想现象（phantom phenomenon），因为冲突存在于一个“幻想”数据上（一个还未插入的数据）。

&emsp;&emsp;在一条 SQL 语句中，具体的数据项（元组）可能被一条 where 语句中的谓词决定，因此，在同一事务的多次运行之间也可能会使用不同的数据项。

&emsp;&emsp;要认识到在并发控制时仅考虑事务访问的元组是不够的，还需要考虑到事务访问元组所需的信息，比如索引等结构。
