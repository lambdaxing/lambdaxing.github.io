...
&emsp;&emsp;随着文件的增大，顺序索引的查找性能和数据扫描性能都会下降，因此需要频繁地对文件进行重新组织。B+ 树（B+ Tree）索引结构是在数据插入和删除的情况下仍能保持其执行效率的几种使用最广泛的索引结构之一。B+树采用平衡树结构，其中根到叶结点的每条路径的长度相同。每个非叶结点有 ⌈n/2⌉ ~ n 个孩子结点，其中 n 对特定的树是固定的。
...
&emsp;&emsp;注意到，B+树索引不仅采用平衡树结构，同时也采用了搜索树的结构，即内部结点中的指针 Pi 指向一棵所含搜索码值大于等于 Ki-1 且小于 Ki 的子树。 
...
&emsp;&emsp;一个关系拥有多个包含同一搜索码值的记录，即两条或多条记录在索引属性上拥有相同的值，该搜索码称为不唯一搜索码（nonunique search key）。不唯一搜索码在记录删除方面存在效率问题，可以通过创建包含原始搜索码和其他属性的复合搜索码来确保搜索码唯一，这也通常被大多数数据库系统使用。

&emsp;&emsp;B+ 树的插入和删除操作复杂，但需要的 I/O 操作较少。在最坏情况下，一次插入/删除的 I/O 操作数可能正比于以 n/2 为底 N 的对数，其中 n 是结点中的指针的最大值，N 是索引文件中的记录条数。换句话说，以 I/O 操作来衡量的插入和删除操作的代价是和 B+ 树的高度成正比的，因此是比较低的，这也使得 B+ 树在数据库实现中成为频繁使用的索引结构。
